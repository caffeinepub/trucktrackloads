...
export default function AdminTransporterDetailsDialog({
  ...
  const handleUpdate = async () => {
    try {
      ...
      await updateTransporter.mutateAsync({ transporter: transporterPrincipal, details: updatedDetails });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            View and manage transporter information, documents, and verification status
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Documents ({transporterDetails.documents.length})</Label>
            {transporterDetails.documents.length === 0 ? (
              <p className="text-sm text-muted-foreground">No documents uploaded</p>
            ) : (
              <div className="space-y-2">
                {transporterDetails.documents.map((doc, idx) => (
